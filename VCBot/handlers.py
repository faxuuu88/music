...
async def on_end_handler(client, update: Update):
   if isinstance(update, StreamAudioEnded):
      ...
      if op==1:
         await bot.send_message(chat_id, "`Sin Música, Dejando Chat De Voz...`")
      elif op==2:
         await bot.send_message(chat_id, "**Se Ah Producido Un Error, Salir Del Cht De Voz...`")
      else:
         await bot.send_message(chat_id, f"**🎧 Nueva Música** \n[{op[0]}]({op[1]}) | `{op[2]}`", disable_web_page_preview=True)
   else:
      pass
